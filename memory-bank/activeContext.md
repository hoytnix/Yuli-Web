...
- Deliberation Tag Firewall invariant: `CCDStreamParser` buffers partial closing tags (`</thought>`) across token chunk boundaries and strips orphan closing tags at stream head. Updated `parseModelResponse` in `src/dialogue/pipeline.ts` to cleanly isolate malformed tag interleaving where thought traces trail the state vector (`</state_vector> [Thought] </thought> [Dialogue]`), routing deliberation traces into the thought drawer while keeping dialogue bubbles pristine.
